...
class GameMines extends JFrame {
    ...
    final int BLOCK_SIZE = 30; // размер 1 блока
    final int FIELD_SIZE = 9; // количество блоков
    final int FIELD_DX = 6; // определяется экспериментально
    final int FIELD_DY = 28 + 17;
    final int START_LOCATION = 200;
    final int MOUSE_BUTTON_LEFT = 1; // для прослушивателя мыши
    ...
    boolean youWon, bangMine; // флаги для победы и взрыва
    int bangX, bangY; // для фиксации координат взрыва
    ...
    GameMines() {
        ...
        canvas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (!bangMine && !youWon) {
                    if (e.getButton() == MOUSE_BUTTON_LEFT) // левая кнопка мыши
                    ...
                    if (bangMine || youWon) timeLabel.stopTimer(); // конец игры
                    canvas.repaint();
                }
            }
        });
    ...
    void openCells(int x, int y) { // рекурсивная процедура открытия ячеек    РЕКУРСИЯ
    ...
    class Cell { // ячейка игрового поля
    ...
    class TimerLabel extends JLabel { // секундомер
        ...
        } // задача TimerTask, длительная задержка, длительный период
